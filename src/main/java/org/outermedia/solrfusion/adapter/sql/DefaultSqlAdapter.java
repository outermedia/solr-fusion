package org.outermedia.solrfusion.adapter.sql;
...
@ToString
@Slf4j
public class DefaultSqlAdapter implements SearchServerAdapterIfc<SolrFusionUriBuilder>
{
    ...
    {
        // TODO handle field boosts, simple add the factor to the score calculation
        // TODO build complete select; where part is in already rendered
        /* Example:
            SELECT
                IF(t2.state = 'Berlin',1,0)+IF(t2.country='Germany',1,0) as score,
                '' as table_t1, t1.*,
                '' as table_t2, t2.*
            FROM de_sball_model_PersonIfc t1
            left join de_sball_model_AddressIfc t2 on t1.address = t2.id
            where (t1.failLoginNr = 0) and (t2.state = 'Berlin' or t2.country='Germany' or true)
            order by score desc;
         */
        ...
        {
            log.error("Caught", e2);
            throw new RuntimeException("Query failed", e2);
        }
    ...
    public SolrFusionUriBuilder buildHttpClientParams(Configuration configuration,
                                                      ...
                                                      FusionRequest fusionRequest, Multimap<String> params,
                                                      Version version) throws URISyntaxException
    {
        ...
        if (!"q".equals(QUERY_PARAMETER))
        {
            ...
            try
            {
                Set<String> defaultSearchFields = fusionRequest
                        .mapFusionFieldToSearchServerField(configuration.getDefaultSearchField(), configuration,
                ...
                String qs = dismaxQueryBuilder
                        .buildQueryString(fusionRequest.getParsedQuery(), configuration, searchServerConfig, locale,
                                defaultSearchFields, QueryTarget.QUERY);
                log.debug("Setting q to dismax query in order to get highlights. Step 1: {}", qs);
                qs = dismaxQueryBuilder
                        .getStaticallyAddedQueries(configuration, searchServerConfig, locale, QueryTarget.QUERY, qs);
    ...
    protected HttpPost newHttpPost(String url,
                                   SolrFusionUriBuilderIfc ub) throws URISyntaxException, UnsupportedEncodingException
    {
